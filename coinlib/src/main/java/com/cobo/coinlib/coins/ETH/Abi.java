...
import static com.cobo.coinlib.Util.sha3;
import static com.cobo.coinlib.coins.ETH.SolidityType.IntType.decodeInt;
...
public class Abi extends ArrayList<Abi.Entry> {
    ...
    public static abstract class Entry {
        ...
        public static class Param {
            ...
            public static List<?> decodeList(List<Abi.Entry.Param> params, byte[] encoded) {
                List<Object> result = new ArrayList<>(params.size());

                int offset = 0;
                for (Abi.Entry.Param param : params) {
        ...
        @JsonCreator
        public static Abi.Entry create(@JsonProperty("anonymous") boolean anonymous,
                                                             @JsonProperty("constant") boolean constant,
                                                             @JsonProperty("name") String name,
                                                             @JsonProperty("inputs") List<Abi.Entry.Param> inputs,
                                                             @JsonProperty("outputs") List<Abi.Entry.Param> outputs,
            ...
            switch (type) {
                ...
                case function:
                case fallback:
                    result = new Abi.Function(constant, name, inputs, outputs, payable);
                    break;
                case event:
                    result = new Abi.Event(anonymous, name, inputs, outputs);
                    break;
            }
    ...
    public static class Constructor extends Abi.Entry {
    ...
    public static class Function extends Abi.Entry {
    ...
    public static class Event extends Abi.Entry {
        ...
        private List<Param> filteredInputs(final boolean indexed) {
            return inputs.stream().filter(param -> param.indexed == indexed).collect(Collectors.toList());
        }
